...
def heartbeat_worker(interval: int = 60):
    ...
    while True:
        ...
        with tracer.start_as_current_span("heartbeat") as span:
            ...
            span.set_attribute("heartbeat.interval", interval)

            span.set_attribute("component", "file-scan-service")

            logger.info("Heartbeat: Service heartbeat")
...
def start_heartbeat(interval: int = 60):
    ...
    thread = threading.Thread(
        target=heartbeat_worker, args=(interval,), daemon=True, name="heartbeat-thread"
    )
