...
def parse_message(message):
    ...
    with tracer.start_as_current_span("parse_message") as span:
        ...
        destination_location = f"{Settings.DEST_CONTAINER}/{file_name}"

        span.set_attribute("file.name", file_name)

        span.set_attribute("scan.result", scan_result)

        return (file_name, scan_result, source_location, destination_location)
...
def copy_blob(file_name):
    ...
    with tracer.start_as_current_span("copy_blob") as span:
        ...
        span.set_attribute("blob.file_name", file_name)
        ...
        logger.info("Starting blob copy: %s", file_name)
...
def delete_source_blob(source_blob, dest_blob, file_name):
    """
    Delete source blob after successful copy.
    """

    with tracer.start_as_current_span("delete_source_blob"):

        if not dest_blob.exists():

            raise RuntimeError("Destination blob missing.")
...
def process_clean_file(
    ...
    with tracer.start_as_current_span("process_clean_file"):
        ...
        source_blob, dest_blob = copy_blob(file_name)

        delete_source_blob(source_blob, dest_blob, file_name)
...
def process_message(message):
    ...
    with tracer.start_as_current_span("processor.process_message") as span:
        ...
        try:
            ...
            processing_time.record((time.perf_counter() - start) * 1000)
