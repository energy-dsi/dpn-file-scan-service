...
def read_secret(path: str) -> str:
    ...
    with tracer.start_as_current_span("auth.read_secret") as span:

        span.set_attribute("secret.file", path)
        ...
        try:
            ...
            logger.exception("Secret file not found.")

            raise RuntimeError(f"Secret file not found: {path}") from ex
...
def get_credential():
    ...
    with tracer.start_as_current_span("auth.get_credential") as span:

        try:

            client_id = read_secret(Settings.CLIENT_ID_FILE)
